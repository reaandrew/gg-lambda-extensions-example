...
/**
 * Simple JSON chunker for large content
...
async function gitguardianMultiscan(contentOrDocs, apiKey, filename = "document.txt") {
  ...
  // Make the API call
  const resp = await fetch(GG_ENDPOINT, {
    method  : 'POST',
    headers : {
...
async function scan(content, apiKey, options = {}) {
  ...
  try {
    ...
    console.log('[GitGuardian] Scan results:', JSON.stringify(results));
...
export { gitguardianMultiscan, buildDocuments, redactSensitiveContent, scan };